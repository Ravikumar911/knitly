...
export async function checkUserHasData(userId: string): Promise<{
  ...
  if (!syncData || !syncData.lastSyncAttemptAt) {
    ...
    syncStatus &&
    ["in_progress", "syncing", "counting_emails"].includes(syncStatus)
  ) {
...
export async function getUnifiedSyncState(
  ...
  if (row.syncTimeoutAt && Date.now() > row.syncTimeoutAt.getTime()) {
    ...
    if (activePhases.includes(row.syncStatus || "")) {
      ...
      await markSyncFailed(
        userId,
        "Sync timed out after 30 minutes. Please try again.",
      );
      return {
        state: "sync_failed",
        phase: "failed",
  ...
  const ACTIVE_PHASES: Array<UnifiedEmailSyncState["phase"]> = [
    "counting_emails",
    "in_progress",
    "syncing",
  ];
  ...
  const updatedAt =
    row.updatedAt ?? row.lastSyncAttemptAt ?? row.lastSyncedAt ?? null;
  const isActive = ACTIVE_PHASES.includes(rawPhase);
  const isStale =
    isActive && updatedAt ? Date.now() - updatedAt.getTime() > STALE_MS : false;
  ...
  if (!row.lastSyncAttemptAt) {
    ...
    ["in_progress", "syncing", "counting_emails"].includes(row.syncStatus || "")
  ) {
...
export async function updateSyncProgress(
  ...
  if (
    ...
    newProcessed < status.totalEmails
  ) {
    const startTime =
      status.lastSyncAttemptAt ?? status.createdAt ?? new Date();
  ...
  await db
    ...
    .set({
      ...
      progressPercentage:
        status.totalEmails && status.totalEmails > 0
      ...
      syncStatus:
        status.totalEmails && newProcessed >= status.totalEmails
          ? "complete"
          : "syncing",
      hasInitialSync:
        status.totalEmails && newProcessed >= status.totalEmails
...
export async function updateSyncStatus(
  ...
  if (existing.length > 0) {
    ...
    return await db.insert(emailSyncStatus).values(insertData).returning();
  }
}
 ...
 */
export async function updateNextPageToken(
  userId: string,
  nextPageToken: string | null,
) {
...
export async function markSyncInProgress(
  ...
  const existing = await db
    .select({ id: emailSyncStatus.id, syncStatus: emailSyncStatus.syncStatus })
    .from(emailSyncStatus)
    .where(eq(emailSyncStatus.userId, userId))
    .limit(1);
 ...
 */
export async function getUsersNeedingSync(): Promise<
  Array<{
    userId: string;
    lastSyncedAt: Date | null;
  }>
...
export async function clearOAuthErrors(userId: string): Promise<void> {
  ...
  if (
    ...
    ["in_progress", "syncing", "counting_emails", "failed"].includes(
      currentStatus.syncStatus,
    )
  ) {
    console.log(
      `Not clearing OAuth errors for user ${userId} - sync status is ${currentStatus.syncStatus}`,
    );
    return;
  }
 ...
 */
export async function resetSyncStatusAfterReauth(
  userId: string,
): Promise<void> {
  await db
    .update(emailSyncStatus)
