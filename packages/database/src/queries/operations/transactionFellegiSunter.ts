...
export async function markDuplicates(
  ...
  for (const [blockingKey, blockTransactions] of Object.entries(blocks)) {
    ...
    for (let i = 1; i < blockTransactions.length; i++) {
      ...
      const paymentLast4Match = checkPaymentMethodLast4Match(
        canonical,
        candidate,
      );
 ...
 */
function groupTransactionsByBlockingKey(
  transactions: Transaction[],
): Record<string, Transaction[]> {
  const blocks: Record<string, Transaction[]> = {};

  for (const txn of transactions) {
    const paymentMethod = txn.paymentMethod as
      | { last4?: string }
      | null
      | undefined;
 ...
 */
function checkPaymentMethodLast4Match(
  tx1: Transaction,
  tx2: Transaction,
): boolean | null {
...
function computeLogOddsScore(tx1: Transaction, tx2: Transaction): number {
  ...
  if (
    tx1.merchantCode &&
    tx2.merchantCode &&
    tx1.merchantCode === tx2.merchantCode
  ) {
  ...
  if (tx1.referenceIds && tx2.referenceIds) {
    ...
    if (
      !hasReferenceMatch &&
      Object.keys(refs1).length > 0 &&
      Object.keys(refs2).length > 0
    ) {
 ...
 */
async function updateDuplicateReferences(
...
export async function markDuplicatesForUser(userId: string): Promise<number> {
  ...
  console.log(
    `User ${userId}: Processing ${userTransactions.length} transactions for duplicates`,
  );
  ...
  const afterCount = updatedTransactions.filter(
    (txn) => !txn.duplicateOf,
  ).length;

  const duplicatesFound = beforeCount - afterCount;
  console.log(
    `User ${userId}: Found ${duplicatesFound} duplicates out of ${beforeCount} transactions`,
  );
