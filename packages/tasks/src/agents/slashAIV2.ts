...
function buildPrompt(basePrompt: string, emailData: EmailData): string {
  const attachments = emailData.attachments
    ?.map((attachment) => {
      const content = attachment.content ? `\nContent: ${attachment.content}` : "";
      return `- ${attachment.filename} (${attachment.mimeType})${content}`;
    })
    .join("\n");
...
Extract all relevant financial data according to the schema.`;
}

function validateExtractionData(
  data: unknown,
): { extractionResult: SwiggyExtractionResult | null; errors: string[] } {
...
export async function extractEmailData(
  ...
  try {
    const { object } = await generateObject({
      model,
      prompt: buildPrompt(SwiggyMerchant.prompt, emailData),
    ...
    if (options.storeTransaction && extractionResult.transaction && emailData.userId) {
      const stored = await storeTransactionV2Input({
        ...
        description: extractionResult.transaction.description || emailData.subject,
        category: "Food",
    ...
    if (NoObjectGeneratedError.isInstance(error)) {
      log.error("Local model response did not match the Swiggy schema", error.text);
    } else {
...
export const slashAIV2Agent = async (emailData: EmailData): Promise<SlashAIV2Result> => {
  return extractEmailData(emailData, defaultModel(), { storeTransaction: true });
};
