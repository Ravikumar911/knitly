...
// Define the schema for extracted financial data
export const ExtractedFinancialData = z.object({
  ...
  transaction: z.object({
    ...
    category: z.enum([
      ...
      'SUBSCRIPTION',
      'OTHER'
    ]).optional().describe("The category of spending this transaction represents"),
...
export interface EmailData {
  ...
  attachments?: Array<{
    filename: string;
    mimeType: string;
    storagePath: string; // Format: attachments/${userId}/${messageId}/${timestamp}_${safeName}
    content?: string; // Base64 encoded content for PDFs
  }>;
} 