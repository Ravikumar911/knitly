...
export const PhonePeTransactionSchema = BaseTransactionSchema.extend({
  ...
  upiId: z.string().optional()
    .describe("UPI ID used for the transaction"),
  ...
  bankAccount: z.object({
    accountNumber: z.string().optional().describe("Masked bank account number"), 
    ifscCode: z.string().optional().describe("IFSC code of the bank"),
    bankName: z.string().optional().describe("Name of the bank")
  }).optional().describe("Bank account details involved in the transaction"),
  ...
  // Transaction mode
  mode: z.enum(["UPI", "WALLET", "CARD", "BANK_TRANSFER"]).optional()
...
export const PhonePeExtractionSchema = BaseExtractionSchema.extend({
  ...
  transaction: PhonePeTransactionSchema.optional()
    .describe("Detailed PhonePe transaction information"),
  
  // PhonePe-specific metadata
  phonePeMetadata: z.object({
    ...
    transactionType: z.enum([
      ...
      "WALLET_LOAD",
      "WITHDRAWAL"
    ]).optional().describe("Type of PhonePe transaction - P2P for person to person, P2M for merchant payments"),
    isSuccessful: z.boolean().default(true)
      .describe("Whether the transaction was successful"),
  }).optional().describe("PhonePe-specific metadata that should be at the root level, not inside transaction"),
});
