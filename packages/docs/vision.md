...
> **Revision on 2026-04-23.** The Gmail access story shipped end-to-end: IMAP + a user-issued Gmail app password (ADR-024) replaces the original `gws` + `gcloud` path (ADR-004, ADR-011, ADR-022), and the `@clack/prompts` wizard (ADR-025) replaces the original readline-only script. The next pivot, captured in [`roadmap/pdf-extractor.md`](./roadmap/pdf-extractor.md), splits PDF extraction out of Gemma into a deterministic local Python library (Docling, ADR-026) invoked per PDF via `child_process.spawn` against a venv bootstrapped by `slashcash doctor --fix` (ADR-027). Gemma keeps chat, email-body extraction, and the reconciliation pass that merges the body candidate with the Docling candidate. The rest of this document — local-first posture, loopback auth boundary, single-user, Ollama, `~/.slashcash/`, no telemetry — is unchanged. Paragraphs below that still mention `gws` are kept for historical continuity; treat them as superseded where they conflict.
