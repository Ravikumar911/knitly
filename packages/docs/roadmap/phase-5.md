...
- Verification: a post-bundle test boots `slashcash start` against the produced bundle, asserts the dual-mode detection picks "tarball mode", asserts the dashboard answers, asserts no dev-only files (`tsx`, source maps for non-prod chunks) shipped. The current local guard starts with `bundle:pack-smoke`: it installs the packed `.tgz` into a temporary npm prefix and verifies that the packaged dashboard includes `.next/BUILD_ID` and can resolve its runtime modules from the installed package layout.
...
What shipped under Phase 5 so far: `.github/workflows/release.yml` (tag-triggered publish with provenance + SBOM + checksum + published-version smoke), `packages/cli/scripts/bundle-app.mjs`, `bundle:check`, `bundle:pack-smoke`, `pnpm pack:local`, bundle checks for required runtime dependencies and symlinks that escape `dist/app`, the `slashcash logs` reader, rotation-aware `LogEvent` writer under `packages/cli/src/runtime/log.ts`, `packages/e2e-tests/bench/perf-budget.ts`, `eval:gate` wiring, `phase-5.ts` scenario (fixture mode). Local dogfood on 2026-04-17 found and fixed two packed-CLI runtime dependency gaps: the global tarball could not load `drizzle-orm` during onboard's SQLite migration, then `slashcash start` could not resolve `next` from the installed dashboard server. `packages/cli/package.json` now declares the bundled database/tasks/dashboard runtime dependencies (`drizzle-orm`, `better-sqlite3`, `dotenv`, `ai`, `@ai-sdk/openai-compatible`, `next`, `react`, `react-dom`), `bundle-app` excludes those installed-runtime packages from the copied Next standalone tree so npm installs the user's ABI-compatible and pack-safe copies, strips the copied Next `.gitignore` so `.next` is actually packed, `bundle:check` fails if these rules drift, and `bundle:pack-smoke` installs the packed tarball into a temporary npm prefix to catch missing `.next` output or runtime modules after npm has transformed the package.
...
- [ ] Run `npm i -g slashcash@<version>` on a **clean** macOS machine (no workspace checkout) and complete `slashcash onboard` from the published tarball. This is the only way to exercise the dual-mode `start` detection path against real installed layout.
  - Partial local coverage on 2026-04-17: `pnpm pack:local`, `bundle:pack-smoke`, `npm i -g packages/cli/slashcash-0.1.0.tgz`, and `slashcash onboard` completed on the maintainer's already-configured machine, including SQLite migration. This does not replace the clean-machine published-package check.
