...
- [x] Pass `--brief --update-adc=false` to `gcloud auth login` and `--scopes gmail.readonly` to `gws auth login` so consent is single-scope and the post-login terminal spam is minimised.
