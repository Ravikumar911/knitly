...
The Google auth sequence (`gcloud-install` → `gcloud-auth` → `gws-install` → `gws-setup` → `gws-login`) implements ADR-022. `gcloud-install` shells `brew install --cask google-cloud-sdk`. `gcloud-auth` delegates to `gcloud auth login`. `gws-setup` runs `gws auth setup`, which reuses the active gcloud credentials to create/select a Google Cloud project, enable the Gmail API, create a Desktop-type OAuth client, add the signed-in account as a test user, and write `~/.config/gws/client_secret.json`. `gws-login` runs `gws auth login --services gmail --readonly`. Splitting `gws-setup` and `gws-login` matters for idempotency: if a previous onboard failed at the login consent, re-running lets us skip the expensive project/client creation and go straight to consent.

### Privacy disclosure inside the wizard

Per vision principle 11 ("trust is surfaced, not buried"), the wizard prints plain-language privacy facts at four moments and one setup-safety note before upstream `gws auth setup`. The copy lives in **one** file, `packages/cli/src/privacy/copy.ts`, exported as named constants so the wizard and pre-consent hooks reuse exact strings, while `slashcash privacy` (see W5) reuses the standing banner. Tests in `packages/cli/src/privacy/copy.test.ts` snapshot each constant so changes to user-facing trust claims or consent instructions show up in review diffs.
...
3. **Right before `gws-setup.install` delegates to upstream setup.** One safety note, printed before it execs `gws auth setup`, because upstream may otherwise run a broad OAuth login before slashcash can narrow access:
...
4. **Right before `gws-login.install` opens a browser.** One line, printed before it execs `gws auth login --services gmail --readonly`:
...
5. **Final-summary block.** Printed by `final-summary.install` after every step is green. Restates what now exists and what does not exist anywhere else:
...
The banner honours `--non-interactive` and `--yes` (it still prints; it does not prompt) and is suppressed under `SLASHCASH_E2E=1` so test logs stay clean. None of the privacy moments or the setup-safety note ask for acknowledgement — this audience hates "I understand" checkboxes (see vision → "Target audience for v1"). They are there to earn trust and keep consent narrow, not to gate it.
...
- `auth-invalid-scope` — fix is `gws auth login --services gmail --readonly`, which lets upstream `gws` spell the real Gmail read-only OAuth scope instead of slashcash passing a raw shorthand.
...
- `not-authenticated` — fix is `gws auth login --services gmail --readonly` interactively.
...
- [x] Pass `--brief --no-update-adc` to `gcloud auth login` and `--services gmail --readonly` to `gws auth login` so consent is Gmail-only/read-only and the post-login terminal spam is minimised.
  ...
  - Live correction on 2026-04-17: `gws auth login --scopes gmail.readonly` reached Google as the invalid raw OAuth scope `gmail.readonly` and produced `Error 400: invalid_scope`; slashcash now uses `gws auth login --services gmail --readonly`, which is the upstream-supported service filter form.
- [x] Implement the new ADR-022 steps in `packages/cli/src/onboard/run.ts`: `gcloud-install`, `gcloud-auth`, and split the existing `gws-auth` step into `gws-setup` (runs `gws auth setup` or the scripted-gcloud fallback) and `gws-login` (runs `gws auth login --services gmail --readonly`). Add the matching detect rules from W1.
...
- [x] Extend the `GwsErrorCode` union with `api-not-enabled`, `gcloud-missing`, and `gcloud-not-authenticated`; wire the classifier signatures (`accessNotConfigured`, empty `gcloud auth list`, exit code 127) and the doctor repair table from W2.
  - Regression fixed on 2026-04-18: the CLI-side doctor diagnostics already had these codes, but the `packages/tasks/src/utils/gws-errors.ts` classifier used by `slashcash sync` had drifted and still lacked `api-not-enabled`, `gcloud-missing`, and `gcloud-not-authenticated`. The tasks wrapper now classifies those signatures too, and the Phase 4 gws integration spec covers every documented `GwsErrorCode` without invoking a real `gws` binary.
- [!] Run a real blank-machine `slashcash onboard` end-to-end: Homebrew → Ollama install → `ollama pull` → gcloud install → `gcloud auth login` → gws install → `gws auth setup` → `gws auth login --services gmail --readonly` → state dir → DB migrate → bundled skill install.
  ...
  - Partial live local coverage on 2026-04-17: after installing the local packed tarball with `npm i -g packages/cli/slashcash-0.1.0.tgz`, `slashcash onboard` completed on the maintainer's already-configured machine. It verified Gmail read-only `gws` auth, migrated `/Users/ravikumarr/.slashcash/db.sqlite`, installed the bundled skill, and printed the final summary. This is not a clean-machine pass and does not satisfy this `[!]` item.
...
- [x] **Ship the privacy disclosures from W1 → "Privacy disclosure inside the wizard".** Land `packages/cli/src/privacy/copy.ts` as the single source of truth with five exported constants (`TOP_BANNER`, `PRE_GCLOUD_AUTH`, `PRE_GWS_SETUP`, `PRE_GWS_LOGIN`, `FINAL_SUMMARY`). Wire `TOP_BANNER` into the onboard runner after `model-question`. Wire `PRE_GCLOUD_AUTH`, `PRE_GWS_SETUP`, and `PRE_GWS_LOGIN` into the matching step's `install` hook so each line prints immediately before the shell-out that opens the browser or delegates to upstream setup. Wire `FINAL_SUMMARY` into the final-summary step. Suppress all five under `SLASHCASH_E2E=1`; still print under `--yes` and `--non-interactive`. Snapshot-test each constant in `packages/cli/src/privacy/copy.test.ts` so changes to user-facing trust claims and consent instructions are visible in review.
