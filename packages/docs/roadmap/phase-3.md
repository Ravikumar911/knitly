...
Per vision principle 11 ("trust is surfaced, not buried"), the wizard prints plain-language privacy facts at four moments. The copy lives in **one** file, `packages/cli/src/privacy/copy.ts`, exported as named constants so the wizard, the pre-consent hooks and the `slashcash privacy` command (see W5) all render the same text. Tests in `packages/cli/src/privacy/copy.test.ts` snapshot each constant so changes to user-facing trust claims show up in review diffs.
