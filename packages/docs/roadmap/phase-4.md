...
- [!] Enforce the real coverage floors: **80%** for `packages/cli`, `packages/database`, `packages/tasks`; **70%** for `apps/main`; **60%** for `packages/ui`. Fail CI on regression.
  - Blocked on the coverage-producing W3/W4/W5 test work being landed first. Measured on 2026-04-18 with `pnpm --filter slashcash --filter @workspace/tasks --filter @workspace/database --filter @workspace/ui --filter @knitly/main test:coverage`: statement coverage was `slashcash` 9.85%, `@workspace/tasks` 8.69%, `@workspace/database` 9.09%, `@workspace/ui` 0.03%, and `@knitly/main` 1.67%. Flipping the documented floors now would make CI permanently red without adding the promised test coverage. This pass did fix Vitest discovery to ignore copied `.next` output so app tests are not double-discovered from bundled standalone trees.
