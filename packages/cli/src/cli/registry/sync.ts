...
import { BUNDLED_GMAIL_SWIGGY_SKILL, installBundledSkills, isSkillEnabled } from "../../skills/registry.js";
...
export function register(program: Command) {
  program
    ...
    .option("--limit <limit>", "Maximum messages to inspect", (value) => Number(value))
    .action(async (options: { full?: boolean; query?: string; limit?: number }) => {
      const config = loadConfig({ createIfMissing: true });
      const paths = resolvePaths();

      installBundledSkills();
      if (!isSkillEnabled(BUNDLED_GMAIL_SWIGGY_SKILL)) {
        throw new Error("The gmail-swiggy skill is disabled. Run `slashcash skills enable gmail-swiggy` to sync Gmail.");
      }
      ...
      process.env.SLASHCASH_GMAIL_QUERY = options.query || config.sync.gmailQuery;
      process.env.SLASHCASH_SYNC_LIMIT = String(options.limit || config.sync.maxMessages);
      ...
      if (result.skipped) {
        console.log(pc.yellow("A sync is already running. Skipped this request."));
        return;
      }

      console.log(pc.green(`Sync complete: ${result.processedCount} processed, ${result.skippedCount} skipped, ${result.errorCount} failed.`));
    });
}
