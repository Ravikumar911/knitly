...
export function SyncInitiator() {
  ...
  const { state, isLoading, statusLabel, statusDescription, cta, refetch } =
    useEmailSync();
  ...
  const getStatusIcon = () => {
    if (requiresReauth)
      return <Lock className="h-8 w-8 text-amber-600 dark:text-amber-500" />;
    if (isComplete)
      return (
      ...
      <Card className="w-full max-w-2xl mx-auto shadow-lg">
        <CardHeader className="text-center pb-4">
          ...
          <CardDescription className="text-base text-muted-foreground max-w-md mx-auto leading-relaxed">
            {statusDescription ||
              "Load local sample transactions and start exploring your spending patterns."}
          </CardDescription>
        </CardHeader>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">
                  {state.progress.processed} of {state.progress.total} records
                  processed
                </span>
                {state.progress.eta && (
                  <span className="text-muted-foreground">
              ...
              <Progress value={state.progress.percent} className="w-full h-2" />
