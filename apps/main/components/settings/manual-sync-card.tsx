...
export function ManualSyncCard() {
  ...
  const hasTotalEmails =
    !!state?.progress.total && (state.progress.total || 0) > 0;
  ...
  const getStatusIcon = () => {
    ...
    if (isActiveSyncInProgress || isLoading)
  ...
  const getStatusDescription = () => {
    ...
    if (isCountingEmails)
      return "Checking the local database for sample records";
    if (isInProgress) return "Preparing local records...";
    if (isSyncing)
      return "Processing local records in the background. You can safely navigate away from this page.";
            ...
            {hasTotalEmails && state && (
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span>
                    {state.progress.processed.toLocaleString()} of{" "}
                    {(state.progress.total || 0).toLocaleString()} records
                    processed
                  </span>
                ...
                <Progress value={state.progress.percent || 0} className="h-3" />
              </div>
            )}
            ...
            {hasTotalEmails && !isComplete && (
              <div className="text-center p-4 bg-primary/10 border border-primary/20 rounded-lg">
                <p className="text-sm text-primary font-medium">
                  Found {(state?.progress.total ?? 0).toLocaleString()} records
                  to process
                </p>
              </div>
            )}
          ...
          {isComplete ? (
            ...
            <div className="text-center">
              <div className="text-sm text-muted-foreground">
                {isSyncing
                  ? "Processing local records in background..."
