...
**How coverage is observed**: `pnpm qa:ingest` validates this inventory against the committed fixture/scenario shape, and `pnpm e2e:ingest` proves the committed fixtures through the real pipeline. Orchestrator/ingest-edge-sweep already ledger `noop_gap` for missing fixture variants such as Instamart.
